...
public class ProcessExecutionService {
    ...
    public static interface EntityStreamSupplier<T> extends Supplier<Stream<T>>{
		...
		public static <T> EntityStreamSupplier<T> of(Supplier<Stream<T>> source){
			return source::get;
		}
...
//
    	...
    	public default <U, E extends Throwable> EntityStreamSupplier<U> map(ThrowableFunction<T, U, E> map){
		...
		default EntityStreamSupplier<T> after(Runnable run){
			Objects.requireNonNull(run, "Before runnale can not e null");
			EntityStreamSupplier<T> me = this;
			return new EntityStreamSupplier<T>(){
				@Override
...
//    }
    ...
    public class Process<T>{
        private Consumer<T> consumer = (t) ->Stream.empty();
        private EntityStreamSupplier<T> supplier = () ->Stream.empty();
...
//    		this.supplier=supplier.before(r);
		...
		public ProcessBulder<T> after(Runnable r){
//    		this.supplier=supplier.before(r);
			this.after = r;
			return this;
		}
