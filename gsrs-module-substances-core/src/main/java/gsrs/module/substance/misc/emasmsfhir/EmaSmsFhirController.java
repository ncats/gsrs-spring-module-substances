...
public class EmaSmsFhirController {
    ...
    public ResponseEntity<?> makeSimpleEmaSmsRecord(@PathVariable("id") String id) {
        ...
        try {
            ...
            return new ResponseEntity<>(jsonEncoded, headers, HttpStatus.OK);
        } catch (Exception e) {
    ...
    public ResponseEntity<?> makeEmaSmsSubstanceDefinition(@PathVariable("id") String id) {
        ...
        try {
            ...
            return new ResponseEntity<>("{\"message\": \"" + message + "Internal error generating FHIR resource\"}", headers, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
}
